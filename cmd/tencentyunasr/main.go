...
func init() {
	...
	flag.StringVar(&secretIds, "SecretId", "", "需要执行任务的 secreId, 有多个则用',' 隔开; 如: -SecretIds=secretId1,secretId2")
	flag.StringVar(&secretKey, "SecretKey", "", "secretId 对应的 key, 必须有! 如: -SecretKey=sfdas111sfas")
...
func getTaskSchedule() schedule.ScheduleInter {
	var (
		...
		tcentAsr handler.TencentAsrHandlerEngin
	)

	return tcentAsr.GetTaskSchedule(resultDir, secretIds, secretKey, concurrentNum)
}
