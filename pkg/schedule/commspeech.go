...
type (
	SyncSchedule struct {
		SecretIds []*recognition.SyncSpeechHdler
		CommSchedule
	}
)

func (sdl *SyncSchedule) closeAllResultCh() {
...
func (sdl *SyncSchedule) concurrentRecognition(url string, concurCtl <-chan emptyStruct) {
	defer func() {
		<-concurCtl
	}()
	wg := sync.WaitGroup{}
